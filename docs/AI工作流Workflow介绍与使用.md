...
| 维度 | review-flow（编排） | bugfix-flow（内联） | dev-flow（编排） |
|------|---------------------|---------------------|------------------|
| 场景 | 存量代码审查+修复 | 单个 Bug 修复 | 从零开发完整功能 |
| 执行主体 | 1 主 + 2 子 | 一个 Agent 干完全部 | 1 主 + 4 子 |
| 特色机制 | 问题分级自选修复、仅报告模式 | 版本化产物、reopen | 批次预计算、全量终检 |
| 人工交互 | 双门禁（方案确认+范围选择） | 三类门禁 | 计划确认门禁 |
...
| 7 | 温度梯度按角色分配 | review-flow 0.2 调度稳定、review-flow-review 0.4 审查发散、review-flow-fix 0.3 修复灵活 |
...
**现状**：工作流假设单人使用，产物目录按日期+功能名天然隔离，多人同时使用不会冲突。

**核心挑战：文件冲突**——与 dev-code 并发执行类似，多人协作时可能修改同一文件：

- 开发者 A 运行 dev-flow 修改 `auth/login.cpp`
- 开发者 B 同时运行 bugfix-flow 修复 `auth/login.cpp` 的同一个 Bug
- 两者合并时可能产生冲突

**优化方向**：

- **共享计划模板**：跨项目复用 plan.md 模板，统一技术选型与架构约定
- **跨会话上下文传递**：通过探索笔记/计划文件实现跨会话、跨开发者的上下文接力
- **冲突检测与预防**：
  - git 分支策略：每人基于 feature branch 工作，合并前检测冲突
  - 产物目录映射：记录每次运行修改的文件清单（modified_files.txt），多人协作时对比清单检测潜在冲突
  - 锁定机制：对高频修改文件（如公共头文件）实施乐观锁，修改前检查是否被他人修改
