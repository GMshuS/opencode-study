...
6. **编写单元测试**（按 plan.md「测试策略」档位分支）：
   - **档位 = 有框架** → 每个核心逻辑任务**必须**配套单元测试，与实现同期交付：
     - **先完成 plan.md「待配置项」**（如补 `scripts.test` / `enable_testing()` / `QT += testlib` / 新建测试项目），再写测试
     - 测试文件位置遵循 plan.md 的位置约定
     - 覆盖正常路径 + 异常路径（空值、越界、重复、权限不足、依赖失败）
     - 外部依赖（网络、数据库、时钟）用替身隔离，不连真实环境
     - 测试文件同样按第 8 项执行 `git add`
   - **档位 = 无框架** → **不写测试、也不声明测试框架依赖**（框架安装由规划阶段负责，失败即本轮不适用测试）
7. 每完成一个文件，记录完整路径
8. **文件暂存**（仅 git 仓库执行，非 git 仓库跳过）：
   - **新增文件**：每新增一个文件后，执行 `git add <文件路径>`
   - **重命名文件**：若需重命名文件，使用 `git mv <旧路径> <新路径>`
   - **删除文件**：若需删除文件，使用 `git rm <文件路径>`
   - 禁止使用 `git add .`，必须显式指定每个文件
9. **每完成一批 Level 任务后，必须立即**更新 `$DOC_PATH/plan.md`：
...
> - 本步骤**只做语法级自检**：不做完整构建、不做全量 Linter

### 单元测试执行（**仅档位 = 有框架时执行**）

调用 `test-verify` skill，传入：
- `command`：取自 plan.md「测试策略」
- `scope`：本批新增 / 改动的测试
- `timeout`：120

处置：
- `❌ N 项失败` → **当场修复后继续**，未修复不交付本批
- `⏭️ 阻断` → 不视为编码失败，记入 `code.md`「待补充内容」，继续，不阻塞
...
已完成任务：[任务A-1, 任务A-2]
未完成任务：[无]
测试结果：【通过 / N 项失败（已修复）/ 阻断（原因）/ 不适用（无框架）】
跳过任务：[无]
