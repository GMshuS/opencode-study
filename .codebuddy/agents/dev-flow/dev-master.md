...
### 步骤1：探索需求、生成开发计划
- 根据初始化阶段的结果构建 prompt：
  - 若提供了 brainstorm.md → prompt = 原始用户需求 + `brainstorm_path: coding-dev/$FEATURE_NAME/brainstorm.md`
...
- **汇总交付最终成果时，根据 `./coding-dev/$FEATURE_NAME/modified_files.txt` 中的文件列表，生成并展示提交信息（包含：问题来源 / 问题原因 / 修改说明 / 测试建议），同时提示用户可通过 `/git/git-autocommit` 提交变更**
