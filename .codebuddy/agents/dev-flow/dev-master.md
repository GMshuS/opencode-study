...
```shell
echo '{ "iteration": 0, "rework_count": 0, "status": "planning" }' > .coding-dev-state.json.tmp
...
3. 按 Level 顺序分批调用 @dev-code：
   ...
   c. **批次完成判定（机械化）**：dev-code 报告"已完成"后，dev-master 立即对 plan.md 执行 grep 核验本批所有任务是否已变为 `- [x]`：
      - 全部勾选 → 视为本批完成，启动下一批
      - 仍有 `- [ ]` → 视为本批未完成，**在同一批内重新调用 dev-code 完成本批**（不计入 rework_count，作为同批重试）；异常处理遵循"重试上限 2 次"规则
   d. 第一批未全部勾选完成不启动第二批
   e. **将每次 subagent 返回结果追加写入** `./coding-dev/$FEATURE_NAME/code.md`（使用 `>>`，累积多批次的"涉及文件"清单和"任务完成度"）

### 步骤2.5：任务覆盖率核验

所有 Level 批次编码完成后，机械核验 plan.md 任务清单的完成度，防止"任务未实现但状态显示已编码完成"：

1. **解析 plan.md**：
   - 提取所有 `- [ ] 任务X`（未完成）/ `- [x] 任务X`（已完成）
   - 汇总为 `todo_tasks` 和 `done_tasks`

2. **生成核验报告** `./coding-dev/$FEATURE_NAME/coverage_report.md`：
   ```markdown
   # 任务覆盖率核验报告
   - 总任务数：N
   - 已完成：M
...
3. **判定**：
   - **若未完成数 = 0** → 更新状态 `status: "coverage-checked"`，继续步骤 3
   - **若未完成数 > 0**：
     a. 从 `.coding-dev-state.json` 读取 `rework_count`
     b. 若 **rework_count >= 2** → **上报后终止**：
        - 更新状态 `status: "failed"`
        - 上报「任务多次未补齐（已补做 2 轮，仍有遗漏），请人工介入处理，工作区代码已保留供参考」
     c. 否则：
        - 告知 @dev-code 读取 `./coding-dev/$FEATURE_NAME/coverage_report.md` 获取遗漏任务清单
        - 告知读取 `./coding-dev/$FEATURE_NAME/plan.md` 获取遗漏任务的验收条件
        - 告知这是「第 N+1 轮补做」并强调"仅实现这些遗漏任务，不要重复已完成任务，不要修改已完成任务"
        - **将 subagent 返回结果追加写入** `./coding-dev/$FEATURE_NAME/code.md`（`>>` 追加）
        - **rework_count += 1**（使用原子写入 tmp + rename）
        - 更新状态 `status: "reworking"`
        - **返回到步骤 2.5 起点重新核验**（最多 2 轮补做循环，超限执行上报后终止）
...
| 步骤 2.5 核验报告 | `./coding-dev/$FEATURE_NAME/coverage_report.md` | `plan.md` + `code.md` |
| 补做批次（dev-code 再次调用） | 追加写入 `code.md` | `plan.md` + `coverage_report.md` |
...
.coding-dev-state.json 贯穿全流程（所有写入均采用 tmp + rename 原子模式）：
  ...
  任务覆盖率核验通过 → status: "coverage-checked"
  补做中 → status: "reworking"（rework_count+1）
