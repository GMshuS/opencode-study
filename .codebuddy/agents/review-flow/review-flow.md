...
将 review-review 返回的摘要原样展示给用户。提问：**「以上审查结果是否确认？如需调整请说明」**
...
使用 review-review 返回摘要中的问题清单展示给用户：
...
> 以下「review-fix 返回摘要」均指最后一轮修复调用的返回值。

1. **执行验证清单终检**：
   - 使用 review-fix 返回摘要中的「验证清单状态」判断
...
3. **生成提交信息**（仅在有修改时）：调用 `commit-msg-format` skill 生成 `$DOC_PATH/commit-msg.txt`
   骨架：`{type}: {标题}` / `问题来源：..`（单行）/ `修改原因：..`（单行）/ `修改方案：..`（单行）/ `修改说明：` 换行 `1) ..` / `测试建议：` 换行 `1) ..` / `--- MODIFIED FILES ---` / `- {相对路径}`
   - 问题来源：用户原始指令的审查范围（如「审查 src/ 目录」）
   - 修改原因：review-fix 返回摘要「修改原因摘要」
   - 修改方案：返回摘要「修改方案摘要」
   - 修改说明：返回摘要「修改说明摘要」
   - 测试建议：返回摘要「测试建议」
   - 改动文件清单：返回摘要「修改文件列表」（合并去重）
