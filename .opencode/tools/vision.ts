...
// ── Config-file fallback for VISION_* env vars ──
//
// OpenCode's `env` config block is NOT injected into the main process
// `process.env`, so custom tools that read `process.env` directly cannot rely
// on it. As a fallback, read VISION_* from the OpenCode config files
// (~/.config/opencode/opencode.jsonc and opencode.json), which are JSONC and
// may contain `//` or `/* */` comments.
...
// On WSL, paths coming from import.meta.url / os.homedir() may be in Windows
// UNC form (e.g. \\wsl.localhost\Ubuntu\home\gmshus\...). Normalize those back
// to a POSIX path so fs can read them.
function normalizeWsl(p: string): string {
  const s = p.replace(/\\/g, "/")
  const m = s.match(/^\/?\/?wsl\.localhost\/[^/]+(\/.*)$/)
  if (m) return m[1]
  return s
}

// Resolve candidate OpenCode config directories. On some setups (e.g. WSL)
// os.homedir() points at the Windows profile while the real config lives under
// the POSIX home, so we also derive the directory from this tool's own file
// location (it lives at <configDir>/tools/vision.ts) and from $HOME.
function configDirs(): string[] {
  ...
  try {
    const here = normalizeWsl(path.dirname(fileURLToPath(import.meta.url)))
    add(normalizeWsl(path.dirname(here)))
  } catch {
    // import.meta.url unavailable
  }
  if (process.env.HOME) add(normalizeWsl(path.join(process.env.HOME, ".config", "opencode")))
  const home = homedir()
  if (home && home !== process.env.HOME) add(normalizeWsl(path.join(home, ".config", "opencode")))
  if (process.env.USERPROFILE) add(normalizeWsl(path.join(process.env.USERPROFILE, ".config", "opencode")))
  return [...new Set(dirs)]
}
...
let _configEnvCache: Record<string, string> | null = null
function getConfigEnv(): Record<string, string> {
  if (_configEnvCache === null) _configEnvCache = readConfigEnv()
  return _configEnvCache
}

// Prefer real process.env, fall back to OpenCode config `env` block.
function cfgEnv(key: string): string | undefined {
  const fromProcess = process.env[key]
  if (fromProcess) return fromProcess
  return getConfigEnv()[key]
}
...
Override with VISION_API_TYPE=openai|minimax.`,
  ...
  async execute(args) {
    ...
    const apiKey = cfgEnv("VISION_API_KEY")
    const baseUrl = cfgEnv("VISION_API_URL")
    ...
    const apiType = (cfgEnv("VISION_API_TYPE") || "").toLowerCase()
...
const FETCH_TIMEOUT_MS = Number(cfgEnv("VISION_FETCH_TIMEOUT_MS") || 60_000)
...
async function callOpenAI(apiKey: string, baseUrl: string, resolved: string[], question?: string) {
  const model = cfgEnv("VISION_MODEL")
  ...
  const maxTokens = Number(cfgEnv("VISION_MAX_TOKENS") || 4096)
