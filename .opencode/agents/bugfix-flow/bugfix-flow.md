...
### 步骤1：问题分析与方案确认（循环，上限 5 轮）

使用内存变量 `$ROUND_COUNT`（初始值 1，不写入状态文件）跟踪轮次。

**第一轮（$ROUND_COUNT = 1）——完整分析**：
1. 读取用户提供的问题描述和相关上下文信息
2. 阅读关联代码模块，理解业务逻辑
3. 根据问题类型选择合适的分析策略，定位根因（如：运行命令复现、代码逻辑推演、
   静态分析、配置检查、边界条件分析、异常处理检查、并发问题排查、数据类型推断等）
4. 向用户展示详细的修复方案，格式要求如下：
```markdown
# 修复方案
...
## 问题分析过程
[根因定位过程：如代码调用链分析、复现步骤、逻辑推演、静态分析等]
