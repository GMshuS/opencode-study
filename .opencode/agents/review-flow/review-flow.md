...
|------|---------|---------|---------|---------|
| 审查 | review-flow-review | FixPlan.md | 覆盖 | review-targets.txt |
| 修复 | review-flow-fix | FixResult.md | 覆盖 | FixPlan.md |
...
4. 确认用户要审查的目录/文件路径，写入 `$DOC_PATH/review-targets.txt`
5. 以原子方式创建状态文件：
...
2. **将 subagent 返回结果写入** `$DOC_PATH/FixPlan.md`
3. 更新状态文件 `status: "reviewed"`

#### 方案确认（循环，最多 5 轮）

从 FixPlan.md 提取以下摘要展示给用户：
- 审查范围（语言/框架、文件数、审查类型）
- 代码优点
...
- **修改** → 更新状态 `status: "plan-revising"`，收集修改意见，
   重新调用 @review-flow-review 修正 FixPlan.md，循环回到确认步骤
...
从 `$DOC_PATH/FixPlan.md` 向用户展示问题清单列表：
...
2. **将 subagent 返回结果写入** `$DOC_PATH/FixResult.md`
3. 更新状态文件 `status: "fixed"`（iteration = $FIX_ITERATION）

#### 2.3 核验与循环判定

从 FixResult.md 提取关键字段进行核验：
- **修复状态**：若"无法修复" → `status: "failed"`，终止
- **问题 ID**：对照 $SELECTED_ISSUES 与「已修复问题」ID 列表，记录遗漏项
- **编译自检**：检查「验证结果 → 编译自检」字段
- **验证清单**：grep FixPlan.md 中 `- [ ]` 是否全变 `- [x]`

核验全部通过 → `status: "verified"`，进入阶段3。

存在失败项时：
- 从状态文件读取 iteration；若 **iteration >= 2** → `status: "failed"`，展示失败项 + 修改文件列表 + 手动回滚命令提示，上报「多次修复未通过，请人工介入」，终止
- 否则 **iteration += 1**，`status: "fix-retrying"`，展示失败项（仅通知），确定新一轮待修复问题集（遗漏项/验证失败项/本轮全量），调用 @review-flow-fix 覆盖 FixResult.md，回到 2.3 核验

### 阶段3：交付成果

1. **执行验证清单终检**：
   - grep `$DOC_PATH/FixPlan.md` 检查 `- [ ]` 是否全部为 `- [x]`
   - 若仍有 `- [ ]` → 将未通过项记录到 FixSummary.md，标记状态为「待人工确认」
   - 若全部 `- [x]` → 标记状态为「全部通过」

2. **汇总交付文档**：生成 `$DOC_PATH/FixSummary.md`，包含审查概览（会话ID、审查范围、总问题数及分级统计、已修复/未修复、涉及文件列表）和验证结果（验证清单状态、编译/类型检查/Linter 结果）。
...
3. **生成提交信息**（仅在有修改时生成）：
   ...
   文件B
   ```
   - 从 `$DOC_PATH/FixResult.md` 提取「修改文件列表」，合并去重
