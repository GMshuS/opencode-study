...
1. **生成修改文件清单**
   - 从 `code.md`（"涉及文件"行）和 `bugfix.md`（"修改文件列表"段）机械提取所有文件路径，合并去重
   - 写入 `$DOC_PATH/modified_files.txt`

2. **生成提交信息**
   - 从子 agent 产出的报告文件机械提取 commit message 素材（严禁分析源码或 git diff）：
     - 需求概述：从 `plan.md` "需求分析与实现方案"段提取功能名称和描述汇总
     - 修改说明：从 `plan.md` "开发任务"段的描述汇总简明实现说明（面向业务，不涉及代码细节）
