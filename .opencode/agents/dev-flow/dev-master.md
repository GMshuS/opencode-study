...
3. 按 Level 顺序分批调用 @dev-code：
   ...
   f. **全量终检**：所有 Level 批次编码完毕后，对 plan.md 执行全量 grep：
      - 若 `- [ ]` 数为 0 → 更新状态 `status: "coded"`，继续步骤 3
      - 若仍有 `- [ ]` → 反查任务所属批次名，仅对缺失任务的批次重新调用 @dev-code
        （复用 3c 的同批重试规则，不计迭代，上限 2 次）

### 步骤2.3：依赖深度合理性检查

在按 Level 顺序编码前，先对步骤 2 的依赖分层结果做合理性检查：

1. **解析依赖分层结果**，统计层级总数
2. **判定**：
   - 若总层级 ≤ 3 → 跳过告警，正常继续按 Level 顺序执行步骤 2 中第 3 步（按 Level 顺序分批调用 @dev-code）
   ...
   - **不阻断流程**，不要求确认，仅作为记录供后续复盘
   - 建议在流程结束后人工复盘 plan，考虑是否需要拆分任务以降低依赖深度

### 步骤3：审查与BUG修复

1. **审查代码**：调用 @dev-review，告知其读取 `./coding-dev/$FEATURE_NAME/code.md` 获取代码文件清单，读取 `./coding-dev/$FEATURE_NAME/plan.md` 了解验收条件
