...
- **规则优先级** — 本技能的构建规则优先级高于项目文档（AGENTS.md 等）。当项目文档推荐 Release 构建时，构建验证仍应使用 Debug 模式。
...
**强制规则：构建验证始终使用 Debug 模式，不做 Release 构建。**

Debug 构建的优势：
- 启用断言检查（`Q_ASSERT`、`assert`），暴露被优化掉的逻辑错误
- 编译器禁用优化（`-O0` vs `-O2`），错误定位更精确
- 保留调试符号，链接错误能精确定位到源码行
- 启用运行时检查（MSVC `/RTC1`、GCC `-D_GLIBCXX_DEBUG`），捕获越界/未初始化等问题

项目文档中的构建脚本（如 `build_qt.bat`）若默认为 Release，应改用对应的 Debug 脚本（如 `build_qt_debug.bat`）或手动添加 Debug 配置参数（如 `CONFIG+=debug`）。
...
> **已有 Debug 脚本的识别**：如果项目同时提供 Release 和 Debug 构建脚本（如 `build.bat` 和 `build_debug.bat`），优先使用 Debug 脚本。如果没有专用 Debug 脚本但有构建配置文件，在原有命令中添加 Debug 配置参数（如 `CONFIG+=debug`、`--configuration=Debug`、`CMAKE_BUILD_TYPE=Debug`）。
...
- **验证构建模式**：执行编译命令前，确认编译参数中包含 Debug 标志（如 `-Zi`/`-g`，无 `-O2`/`-DNDEBUG`）。如果检测到 Release 标志，应修正为 Debug 后再执行，并在报告中注明修正原因
