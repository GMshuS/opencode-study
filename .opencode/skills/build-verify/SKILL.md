...
- **已有构建脚本/配置**：检查 .vcxproj / .sln / .pro / CMakeLists.txt / Makefile / build.bat / build.sh 等。**有则优先使用，无需继续工具链猜测**
- **构建系统**：（仅当无构建脚本时）识别构建配置文件类型
- **工具链**：（仅当无构建脚本时）检查环境可用工具（`which g++` / `node --version` / `cargo --version` 等）
- **Debug 脚本**：检查是否提供 Debug 专用构建脚本（如 `build_debug.bat`、`Makefile.Debug`）
- **变更规模**：
