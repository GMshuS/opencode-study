...
**加载方式**：执行 `skill({name: "<skill名称>"})` 加载对应的编码规范 skill。

#### 语言特定检查清单
- **Go**: 加载 `go-coding-standards` skill（并发安全、错误处理、性能优化）
- **Python**: 加载 `python-coding-standards` skill（类型注解、异常处理、资源管理）
- **JavaScript/TypeScript**: 加载 `javascript-coding-standards` skill（异步处理、类型安全、模块规范）
- **C/C++**: 加载 `c-cpp-coding-standards` skill（内存管理、指针安全、RAII）
- **其它**: 参照**通用审查维度**
